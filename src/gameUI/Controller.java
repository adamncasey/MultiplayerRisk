...
public class Controller implements Initializable {
	...
	ArmyMode armyMode;
	CenterPaneMode centerPaneMode = CenterPaneMode.GAME;
	...
	public void initialize(URL url, ResourceBundle resourceBundle) {
		...
		ArrayList<GUITerritory> highlighted_all = default_map
				.getTerritoryList();
	...
	public void set_armies(int playerID, int number, GUITerritory territory) {
		...
		printToConsole(territory.getImage().getId() + " " + x + " " + y + " "
				+ sizex + " " + sizey);
	...
	void setCenterPaneMode(CenterPaneMode newMode) {
		if (centerPaneMode != newMode) {
			...
			case GAME: {
				centerPane.getChildren().setAll(gamePane);
				centerPaneMode = newMode;
				break;
			}
			}
	...
	final ChangeListener<Number> listener = new ChangeListener<Number>() {
		...
		final long delayTime = 200; // delay that has to pass in order to
